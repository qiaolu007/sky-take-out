...
public class OrderServiceImpl implements OrderService {
    ...
    public void rejectOrder(OrdersRejectionDTO ordersRejectionDTO) throws Exception {
        ...
        if (payStatus.equals(Orders.PAID)) {
            // 调用微信支付退款接口
            String refund = weChatPayUtil.refund(ordersDB.getNumber(), // 商户订单号
                    ordersDB.getNumber(), // 商户退款单号
                    new BigDecimal(0.01),// 退款金额，单位 元
                    new BigDecimal(0.01));// 原订单金额

            log.info("商家拒单-申请退款：{}", refund);
        }
    ...
    public void cancelOrder(OrdersCancelDTO ordersCancelDTO) throws Exception {
        ...
        //订单状态 1待付款 2待接单 3已接单 4派送中 5已完成 6已取消
        if (ordersDB == null || ordersDB.getStatus() < Orders.CONFIRMED || ordersDB.getStatus() > Orders.COMPLETED) {
            throw new OrderBusinessException(MessageConstant.ORDER_STATUS_ERROR);
        }
    ...
    private void checkOutOfRange(String address) {
        ...
        // 获取店铺的经纬度坐标
        String shopCoordinate = HttpClientUtil.doGet("https://api.map.baidu.com/geocoding/v3", paramMap);
        ...
        String lat = location.getString("lat"); // 店铺纬度值
        String lng = location.getString("lng"); // 店铺经度值
        //店铺经纬度坐标
        ...
        if (address != null && !address.isEmpty()) {
            ...
            // 获取用户收货地址的经纬度坐标
            String userCoordinate = HttpClientUtil.doGet("https://api.map.baidu.com/geocoding/v3", paramMap);
            ...
            lat = location.getString("lat"); // 店铺纬度值
            lng = location.getString("lng"); // 店铺经度值
            //用户收货地址经纬度坐标
            userLngLat = lat + "," + lng;
        }
        ...
        // 路线规划
        String json = HttpClientUtil.doGet("https://api.map.baidu.com/directionlite/v1/riding", paramMap);
